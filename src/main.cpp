...
//TODO: this isn't identical to argentum reference client.
...
{	
if(nHeight <= Version4StartHeight){
...
static const int64_t nTargetTimeSpan2 = 60 * 250; // Argentum: Every 250 blocks
static const int64_t nTargetSpacing2 = 60; // Argentum: 60 Seconds
...
unsigned int static DarkGravityWave3(const CBlockIndex* pindexLast, const CBlock *pblock, int algo) {
...
unsigned int static DarkGravityWave_Upd(const CBlockIndex* pindexLast, const CBlock *pblock, int algo) {
    /* current difficulty formula, darkcoin - DarkGravity v3, written by Evan Duffield - evan@darkcoin.io */
    const CBlockIndex *BlockLastSolved = pindexLast;
    const CBlockIndex *BlockReading = pindexLast;
    const CBlock *BlockCreating = pblock;
    ...
    int64_t nTargetTimespan = CountBlocks*nTargetSpacing2;
...
{
    return GetNextWorkRequired(pindexLast, pblock, algo);
}
...
void ThreadScriptCheck() {
    RenameThread("argentum-scriptch");
...
bool AcceptBlockHeader(CBlockHeader& block, CValidationState& state, CBlockIndex** ppindex)
{
    ...
    if (hash != Params().HashGenesisBlock()) {
        ...
        // Argentum: reject v2 and v1 blocks at the same time, only check once
        if (block.GetBaseVersion() < 3) {
...
bool AcceptBlock(CBlock& block, CValidationState& state, CBlockIndex** ppindex, CDiskBlockPos* dbp)
{
    ...
    // Argentum: reject ONLY if block.nVersion=3 has a supermajority because CBlockIndex::IsSuperMajority
...
std::string CDiskBlockIndex::ToString() const
{
    ...
    str += strprintf("\n                hashBlock=%s, hashPrev=%s, hashParentBlock=%s)",
        ...
        (auxpow.get() != NULL) ? auxpow->GetParentBlockHash(algo).ToString().substr(0,20).c_str() : "-");
    return str;
}
