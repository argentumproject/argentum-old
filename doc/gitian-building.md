...
*Setup instructions for a gitian build of Argentum using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Argentum
...
- [Building Argentum](#building-argentum)
...
Clone the git repositories for argentum and gitian,
...
In [doc/release-process.md](release-process.md) in the argentum repository under 'Fetch and build inputs'.
...
To build Argentum (for Linux and/or Windows) just follow the steps under 'perform
...
For example:
```bash
URL=https://github.com/langerhans/argentum.git
...
```bash
    gpg --detach-sign ${VERSION}/${SIGNER}/argentum-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/argentum-build.assert
```
...
[argentum/gitian.sigs](https://github.com/argentum/gitian.sigs/) repository, or
